...
| `--audit-db` | optional flag | `pathlib.Path` | `None` | [parallax/cli.py:324-332](../../parallax/cli.py) | Per-invocation override of `PARALLAX_AUDIT_DB_PATH`. Defensive guard (issue #63 RESOLVED): if the resolved path contains the substring `parallax-kernel/db`, the override is rejected **unconditionally** (whether or not `--dry-run` is set) with `reason_code=disk.audit_db_unsafe_path` + `SystemExit(78)` ([cli.py:1046-1071](../../parallax/cli.py)). Protects against accidental production writes — `--audit-db` is documented as a testing-only override. |
...
| `disk.audit_db_unset` | parallax/cli.py:1034-1044 (env-var KeyError → `return 78`; no exception raised) | 78 | `PARALLAX_AUDIT_DB_PATH` env var unset and no `--audit-db` override. The prior binary inconsistency at the `parallax-kernel/db` guard emit site (was tagging `disk.audit_db_unset` while raising `SystemExit(70)`) is RESOLVED in this PR — the guard now uses the dedicated `disk.audit_db_unsafe_path` reason code below + `SystemExit(78)` (see §9.9). |
| `disk.audit_db_unsafe_path` | parallax/cli.py:1046-1071 (unconditional guard → `raise SystemExit(78)`) | 78 | `--audit-db` override resolves to a path containing the substring `parallax-kernel/db`. Defensive guard against accidental production writes; fires regardless of `--dry-run` (issue #63 RESOLVED). Reason code added in 2026-05-20 follow-up PR so the log tag and process exit agree (was the binary inconsistency at issue #64). |

24 reason codes total. The contract spec §6.2 listed 24; the missing one (`signer.fingerprint_mismatch`) is intentionally NOT implemented and is documented as a follow-up at [aphelion_ingest.py:92-101](../../parallax/apex/aphelion_ingest.py) (see §9.1 below). The `disk.audit_db_unsafe_path` code is additive to the contract namespace and was introduced by the issue #63 / #64 follow-up PR to fix a binary-level log↔exit inconsistency.
...
Phase [8] is **atomic at the package level** (issue #65 RESOLVED in 2026-05-20 follow-up PR). `_write_batch` ([aphelion_ingest.py:569-597](../../parallax/apex/aphelion_ingest.py)) delegates to `write_audit_rows_atomic` ([audit_db.py](../../parallax/apex/audit_db.py)), which wraps every row in the batch inside a single `BEGIN IMMEDIATE` / `COMMIT` against an autocommit-state connection. The M5 §4.7 write-order fence is therefore **per package**, not per row: if any row in the batch raises (`sqlite3.IntegrityError` / `AuditDbWriteError` / `PermissionError` / canonicalize failure), `ROLLBACK` runs and `audit.db` is left byte-identical to its pre-call state. `assert_audit_row_committed` still fires per row inside the batch transaction — the §8.1 fence guarantee that envelope assembly cannot precede a successful audit INSERT attempt survives the upgrade, and is strictly tighter now because the rows commit together at the batch boundary rather than one-at-a-time.

Operators no longer need a phase-[8] partial-write recovery procedure for batches that fail inside `write_audit_rows_atomic`: the failed-ingest log line + non-zero exit unambiguously implies zero new audit rows for that `package_id`. The runbook §2.2 recovery section is kept for reference (a `disk.audit_db_write_failed` exit still permits operator-side investigation of the underlying I/O failure), but the partial-state operator workflow is no longer needed for the §9.10 scenario.
...
### 9.8 `--audit-db` defensive guard scope limitation — RESOLVED (issue #63)

**Status (2026-05-20 follow-up PR):** RESOLVED. The guard at [parallax/cli.py:1046-1071](../../parallax/cli.py) was widened to fire on `--audit-db` regardless of `--dry-run`. Any override path containing the substring `parallax-kernel/db` is now rejected unconditionally with `reason_code=disk.audit_db_unsafe_path` + `SystemExit(78)`. The runbook §2.5 operator-side warning is now a defence-in-depth note rather than the primary protection.

**Original report (preserved for history):** The defensive guard previously fired only when BOTH `--audit-db` and `--dry-run` were set. A plain `parallax ingest <pkg> --audit-db /home/chris/parallax-kernel/db/audit.db` (no `--dry-run`) was NOT blocked — the guard did not protect against operator typo on a real production-write invocation.

### 9.9 `cli.py:1064` `SystemExit(70)` + `reason_code=disk.audit_db_unset` binary inconsistency — RESOLVED (issue #64)

**Status (2026-05-20 follow-up PR):** RESOLVED. The widened guard (§9.8) now uses a dedicated reason code `disk.audit_db_unsafe_path` mapped to exit 78 via `_REASON_TO_EXIT` at [aphelion_ingest.py:113](../../parallax/apex/aphelion_ingest.py), and raises `SystemExit(78)` to match. The log tag and process exit agree. The §6.2 reason-code table was updated to add the new code (24 total, up from 23).

**Original report (preserved for history):** The `--audit-db + --dry-run + parallax-kernel/db` guard emitted a structured log line tagged `reason_code=disk.audit_db_unset` but raised `SystemExit(70)`. Per §4.2 and `_REASON_TO_EXIT`, `disk.audit_db_unset` maps to exit 78 (`EX_CONFIG`), not 70 (`EX_SOFTWARE`). An operator who read the log expected exit 78 but the actual process exit was 70.

### 9.10 Phase-[8] partial-batch persistence (codex round-2 finding) — RESOLVED (issue #65)

**Status (2026-05-20 follow-up PR):** RESOLVED. A new sibling helper `write_audit_rows_atomic(conn, rows)` was added to [parallax/apex/audit_db.py](../../parallax/apex/audit_db.py); `_write_batch` ([aphelion_ingest.py:569-597](../../parallax/apex/aphelion_ingest.py)) delegates to it. The helper opens a single `BEGIN IMMEDIATE`, runs every row's belt-and-braces `canonicalize_row` + `INSERT`, then `COMMIT`s the whole batch — any failure (validation, integrity, write error) triggers `ROLLBACK` and `audit.db` is left byte-identical to its pre-call state. `write_audit_row`'s `conn.in_transaction must be False` precondition is preserved by leaving the per-row API untouched and routing batch writes through the new helper. `assert_audit_row_committed` still fires per row inside the batch transaction so the M5 §8.1 fence semantics survive the upgrade. M5 §4.7 fence semantics escalate from **per row** to **per package**.

**Original report (preserved for history):** `_write_batch` iterated audit rows one at a time, calling `write_audit_row` per row; `write_audit_row` wrapped each INSERT in its own `BEGIN IMMEDIATE` / `COMMIT` against an autocommit-state connection. If row N raised mid-batch, rows 1..N-1 were already committed and survived in `audit.db`; the M5 §4.7 write-order fence was **per row**, not per package. The contract spec §2 I-2.3 said "partial-success is rejected" but its scope covered verify_package phases [1]-[6] only — the contract did not specify atomic-batch semantics for the audit-write phase. The runbook §2.2 partial-batch recovery procedure remains for `disk.audit_db_write_failed` general I/O investigation but is no longer the primary remediation for this scenario.
...
| v0.2 | 2026-05-20 | Issue #63 / #64 / #65 follow-up PR. (1) `--audit-db` defensive guard widened to fire unconditionally (no longer dry-run-scoped); new `disk.audit_db_unsafe_path` reason code + `SystemExit(78)`. (2) `cli.py` `SystemExit(70)` ↔ `reason_code=disk.audit_db_unset` log/exit inconsistency resolved by the new dedicated reason code. (3) `_write_batch` upgraded from per-row to per-package atomicity via new `write_audit_rows_atomic` helper; M5 §4.7 fence semantics escalated. §2.1, §4.1, §5.3, §9.8, §9.9, §9.10 sync. |
