...
| Empty corpus — dir IS accessible but contains zero `.aphelion.tar` files | Return empty claim list (this is a legitimate fresh-deploy state); also increment the `parallax_apex_empty_corpus_total` counter on **every** such read so a stuck-empty deployment is visible in dashboards as a sustained *rate* (per the 2026-08-23 amendment in §4.5 — this row originally specified a first-read-per-process boundary that was never implemented) | Facade sees `[]` and proceeds without Apex context |
...
| `parallax_apex_read_errors_total` | Counter | `reason={package_missing, unsigned_package, signer_untrusted, package_corrupt, package_dir_inaccessible, lib_error, audit_write_failure, other}`, `exc_class={<class name>}` for `reason=lib_error` only | Every `AphelionUnreachableError` raise (includes `reason="audit_db_write_failed"` for audit-write failures). This is the canonical M7 error counter — granular per-reason but a single series family. (Earlier draft had a separate `parallax_apex_lib_errors_total`; folded into this counter to avoid double-count ambiguity.) |
...
| `parallax_apex_empty_corpus_total` | Counter | (no high-cardinality labels) | **Every** read where the package directory is accessible but empty (see the 2026-08-23 amendment below); lets ops dashboards detect stuck-empty deploys via its *rate*, not its level |
...
**Amendment 2026-08-23 — `parallax_apex_empty_corpus_total` is per-read, not first-read-per-process.** This spec originally specified the counter (here and in the §4.3 empty-corpus row) as firing once per process, deliberately complementary to `parallax_apex_empty_result_total{cause="empty_corpus"}`. **That first-read-per-process guard was never implemented**: both `EMPTY_CORPUS.inc()` sites in `parallax/apex/router.py` (the exact-subject loader in `_load_candidate_claims` and the index check in `_query_freetext`) increment unconditionally on every read against an empty corpus. Both sections are now aligned to the shipped behaviour rather than the code being changed to match the spec, because no consumer depends on the deploy-boundary semantics: the `ApexStuckEmptyCorpus` rule already alerts on the labelled `empty_result{cause="empty_corpus"}` series, and the M7 read path is not yet wired (#106.2), so no historical data carries the old meaning. Consequences an implementer must know:

- The two series are now **redundant** — `parallax_apex_empty_corpus_total` is the unlabelled twin of `parallax_apex_empty_result_total{cause="empty_corpus"}`. Prefer the labelled series in new alerts so all empty-result causes stay symmetric; the unlabelled one is retained for the stuck-deploy dashboard panel and for compatibility with the §4.5 metric-set contract test.
- **The deploy-boundary detector this counter was specified to provide no longer exists.** A dashboard may not read a *level* ("stuck > 0 across a deploy") off either series, because under per-read semantics both grow monotonically with request rate; read a `rate()`/`increase()` window instead.
- If first-read-per-process semantics are actually wanted, add the guard in a follow-up (a per-process `bool` latch around both `inc()` sites) and revert this amendment — do not reintroduce the wording without the guard.
...
- **Apex error** (`AphelionUnreachableError`, including `reason="audit_db_write_failed"` for audit-write failures): facade logs + surfaces the error to the upstream caller (Claude Code hook); does not silently swap Perihelion content into the Apex slot to hide the failure.
...
| 2026-05-28 | v0.1.3-reframe-2026-05-28 | fix: §4.3 + §4.5 + §8 Q3 exception class name corrected from ghost-reference `AphelionAuditWriteError` to existing `AphelionUnreachableError(reason="audit_db_write_failed")` — reconciles spec with M5 impl (`parallax/router/aphelion_adapter.py`). 4 occurrences replaced. See ultrawork scan 2026-05-28 EVE. |
