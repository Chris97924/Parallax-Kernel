...
| Audit DB path config | `PARALLAX_AUDIT_DB_PATH` env var; defaults: Windows `E:\Parallax\data\audit.db`, Linux ZenBook `/home/chris/parallax-kernel/db/audit.db`. Validation at startup. | **CONFIGURED 2026-05-09** via `docs/m5-prep/audit-db-path-config.md`; Chris-action items §6 still pending (mkdir, env file edit, schema migration) |
